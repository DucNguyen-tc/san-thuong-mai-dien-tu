  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      ...
      if (backendPaymentMethod === 'CASH') {
        navigate(`/payment-result?status=success&orderId=${order.id}&method=CASH`);
        ...
        navigate(`/payment-result?status=success&orderId=${order.id}&method=${backendPaymentMethod}`);
      }
    } catch (error: any) {
      console.error('Lỗi khi thanh toán:', error);
      const msg = error.response?.data?.message || 'Có lỗi xảy ra trong quá trình đặt hàng. Vui lòng thử lại!';
      setErrorMessage(msg);
