...
router.get('/popular', recommendationController.getPopularProducts.bind(recommendationController));

// POST /api/recommendations/batch — Gợi ý tổng hợp từ nhiều sp (hành vi đa dạng)
router.post('/batch', recommendationController.getBatchRecommendations.bind(recommendationController));
